...
action :get do
  ...
  if node['rubrik_org_name'] and node['rubrik_org_name']!='Global'
    ...
    org_id = Rubrik::Api::Organizations.get_single_org_by_name('https://' + node['rubrik_host'], token, node['rubrik_org_name'], timeout)['id']
    if org_id
      org_objects = Rubrik::Api::Organizations.get_org_managable_objects('https://' + node['rubrik_host'], token, org_id, timeout)
...
action :set do
  ...
  if node['rubrik_org_name'] and node['rubrik_org_name']!='Global'
    ...
    if org_id
      ...
      case new_resource.object_type
      when 'vmwarevm'
        object_id = Rubrik::ConfMgmt::Helpers.get_vmware_vm_id('https://'+node['rubrik_host'], token, [node['hostname'],node['ipaddress']], timeout)
        org_objects.each do |org_object|
          assigned_to_org = true if org_object == object_id
        end
      when 'host'
        object_id = Rubrik::ConfMgmt::Core.get_registered_host_id('https://'+node['rubrik_host'],token,[node['hostname'],node['ipaddress']], timeout)
        org_objects.each do |org_object|
          assigned_to_org = true if org_object == object_id
        end
      else
      ...
      if assigned_to_org
        ...
        if !object_id
          ...
          assign_to_org = Rubrik::Api::Organizations.add_object_to_org('https://' + node['rubrik_host'], token, object_id, org_id, timeout)
