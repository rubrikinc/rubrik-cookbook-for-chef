...
action :get do
  ...
  consistency = Rubrik::ConfMgmt::Core.get_vmware_vm_consistency('https://' + node['rubrik_host'], token, [ node['hostname'], node['ipaddress'] ])
...
action :set do
  ...
  current_consistency = Rubrik::ConfMgmt::Core.get_vmware_vm_consistency('https://' + node['rubrik_host'], token, [ node['hostname'], node['ipaddress'] ])
  if new_resource.crash_consistent == true
    if current_consistency == 'CRASH_CONSISTENT'
      ...
      consistency_update = Rubrik::ConfMgmt::Core.set_vmware_vm_consistency('https://' + node['rubrik_host'], token, [ node['hostname'], node['ipaddress'] ], 'CRASH_CONSISTENT')
    ...
    if current_consistency == 'CRASH_CONSISTENT'
      consistency_update = Rubrik::ConfMgmt::Core.set_vmware_vm_consistency('https://' + node['rubrik_host'], token, [ node['hostname'], node['ipaddress'] ], '')
      if consistency_update.nil?
        Chef::Log.error "Something went wrong updating the consistency"
        exit
      end
