...
action :get do
  ...
  is_registered = Rubrik::ConfMgmt::Core.check_host_registered('https://' + node['rubrik_host'], token, [ node['hostname'], node['ipaddress'] ], timeout)
  if is_registered
...
action :set do
  ...
  if is_registered
    ...
    register_host = Rubrik::ConfMgmt::Core.register_host('https://' + node['rubrik_host'], token, [ node['hostname'], node['ipaddress'] ], timeout)
    if register_host
