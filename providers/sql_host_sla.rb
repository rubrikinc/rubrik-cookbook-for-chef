...
action :get do
  ...
  if host_id == false
    ...
    current_protection = Rubrik::ConfMgmt::Core.get_all_sql_instances_protection('https://' + node['rubrik_host'], token, host_id, timeout)
    ...
    if current_protection.length == 0
      ...
      current_protection.each do |key, value|
        if value != node['rubrik_sla_domain']
          instance_id = Rubrik::ConfMgmt::Core.get_sql_instance_id_by_name_and_host_id('https://' + node['rubrik_host'], token, key, host_id, timeout)
          protect_task = Rubrik::ConfMgmt::Core.update_sql_instance_protection('https://' + node['rubrik_host'], token, instance_id, node['rubrik_sla_domain'], new_resource.log_backup_freq_minutes * 60, new_resource.log_retention_days * 24, timeout)
